        ...
        if (spawnData.timeUntilSpawn < 0)
        {
            Job.WithCode(() =>
            {
                ...
                for (int i = 0; i < spawnDataJob.spawnCount; i++)
                {
                    ...
                    Entity enemy = ECS.Instantiate(1, spawnDataJob.enemy);
                    ECS.SetComponent(1, enemy, new Translation { Value = spawnPosition + spawnPositionMod });
                    ECS.SetComponent(1, enemy, new TargetData { targetPosition = targetPosition });
                }
